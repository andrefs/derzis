...
export const allTriples = writable<Array<{
	subject: string;
	predicate: string;
	object: string;
	createdAt: string;
}>>([]);
export const filteredTriples = writable<Array<{
	subject: string;
	predicate: string;
	object: string;
	createdAt: string;
...
export function processTriplesData() {
	...
	allPredicates.set(Array.from(counts.entries())
...
export const effectiveHop = derived([selectedPredicates, currentHop], ([$selectedPredicates, $currentHop]) =>
...
export const computedFilteredData = derived(
	...
	([$allTriples, $selectedPredicates, $effectiveHop, $seeds, $branchFactors]) => {
		...
		return getTriplesWithinHops(sortedTriples, $seeds, $selectedPredicates, $effectiveHop, $branchFactors);
	}
);
...
computedDates.subscribe(({ minDate: min, maxDate: max, minDateLabel: minL, maxDateLabel: maxL }) => {
	minDate.set(min);
	maxDate.set(max);
