...
export const load: PageServerLoad = async ({ params, url }) => {
  ...
  try {
    ...
    const longestPathsCount = await TraversalPath.countDocuments(lpFilter)
      .where('nodes.count')
      .equals(maxNodes);
    const shortestPathsCount = await TraversalPath.countDocuments(spFilter)
      .where('nodes.count')
      .equals(minNodes);
    ...
    const longestTriplesInOrder = longestPath.triples
      .map((tripleId) => {
        const foundTriple = longestTriples.find((t) => t._id?.toString() === tripleId.toString());
        const procTriple = longestProcTriples.find(
          (pt) => pt.triple?.toString() === tripleId.toString()
        );
    ...
    if (shortestPath) {
      ...
      // Sort triples to maintain the same order as in the Path document
      const shortestTriplesInOrder = shortestPath.triples
        .map((tripleId) => {
          const foundTriple = shortestTriples.find(
            (t) => t._id?.toString() === tripleId.toString()
          );
          const procTriple = shortestProcTriples.find(
            (pt) => pt.triple?.toString() === tripleId.toString()
          );
