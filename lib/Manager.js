...
const domainsToCrawl = async function*(workerId, limit){
  for await(const domain of Domain.domainsToCrawl(workerId, limit)){
    //const resources = await Resource.resourcesToCrawl(domain.origin, workerId, config.workers?.jobs?.domainCrawl?.resourcesPerDomain || 100);
    const heads = await Path.find({status: 'active', 'head.domain': domain.origin})
        .select('head')
        .limit(config.workers?.jobs?.domainCrawl?.resourcesPerDomain || 100)
        .lean();
