...
const updateJobResults = async function(data){
  ...
  if(data.jobType === 'resourceCrawl'){
    ...
    await saveCrawlInfo(source, data.results.triples);
  }
};


const saveCrawlInfo = async function(source, triples){
  console.log('XXXXXXXXXXXXXXx 1', util.inspect(triples));
  const insTrips = await Triple.insertMany(triples.map(t => ({
...
const updateRobotsInfo = async function(data){
  ...
  if(data.ok){
    const robots = robotsParser(data.host+'/robots.txt', data.robots);
    crawlDelay = robots.getCrawlDelay(config.userAgent) || crawlDelay;

    doc = {
      '$set': {
        'robots.text': data.robots,
        'robots.checked': data.endTime,
        'robots.elapsedTime': data.elapsedTime,
        'robots.status': 'done',
        status: 'ready',
        'crawl.delay': crawlDelay,
        'crawl.nextAllowed': new Date(data.endTime+(crawlDelay*1000)),
        lastAccessed: data.endTime
      }, '$unset': {workerId: ''}
    };
  }
  else if(data.errorType = 'http'){
    let robot_status = 'error';
    if(data.httpStatus === 404){ robot_status = 'not_found'; }
