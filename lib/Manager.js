...
const updateJobResults = async function(data){
  ...
  if(data.jobType === 'resourceCrawl'){
    if(data.results.ok){
      ...
      await saveCrawlInfo(source, data.results.details.triples);
    }
    // TODO handle errors
  }
};
...
const saveCrawlInfo = async function(source, triples){
  const filtTrips = triples
    .filter(t => t.subject.termType === 'NamedNode' && t.object.termType === 'NamedNode')
    .map(t => ({
  ...
  const insTrips = await Triple.insertMany(filtTrips, {ordered:false});

  const newResources = {};
  for(const t of insTrips){
    if(t.subject === t.source || t.object === t.source){
      newResources[t.subject] = true;
      newResources[t.predicate] = true;
      newResources[t.object] = true;
    }
  }
