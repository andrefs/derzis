...
class Manager {
  ...
  async updateJobResults(data){
    ...
    if(data.jobType === 'resourceCrawl'){
      const objs = data.results.resources.map(u  => ({url: u, domain: new URL(u).origin, crawled: false}));
      await Resource.insertMany(objs, {ordered: false});
    }
  }

  async updateDomainInfo(results){
    let crawlDelay = 1;
