...
const updatePaths = async function(sourceUrl, allTriples){
  let paths = await Path.find({'head.url': sourceUrl, status: 'active'});

  const filter = t => t.subject === sourceUrl || t.object === sourceUrl;
  const triples = allTriples.filter(filter);
...
const domainsToCrawl = async function*(workerId, limit){
  for await(const domain of Domain.domainsToCrawl(workerId, limit)){
    ...
    const heads = await Path.find({status: 'active', 'head.domain': domain.origin})
                            .select('head')
                            .limit(limit)
                            .lean();
