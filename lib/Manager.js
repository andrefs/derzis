...
const updateJobResults = async function(data){
  ...
  if(data.jobType === 'resourceCrawl'){
    if(data.results.ok){
      ...
      await Resource.markAsCrawled(data.url, data.results.details.ts, true);
    }
  }
};
...
const addExistingHead = async function(path){
  ...
  let betterPathCandidates = await Path.findOne({
    'seed.url': path.seed.url,
    'head.url': path.head.url,
    'predicates.count': {'$lt': path.predicates.count}
  });
  betterPathCandidates = betterPathCandidates?.filter(pc => pc.every(n => props.has(n)));
  if(betterPathCandidates && betterPathCandidates.length){
    path.status = 'disabled';
    return path.save();
  }
