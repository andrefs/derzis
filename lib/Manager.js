...
const saveCrawl = async function(sourceUrl, details){
  ...
  if(triples.length){
    ...
    const newTriples = await Triple.find({_id: {'$in': Object.values(res.upsertedIds)}});
...
const domainsToCrawl = async function*(workerId, limit){
  for await(const domain of Domain.domainsToCrawl(workerId, limit)){
    ...
    const heads = await Resource.find({domain: domain.origin, status: 'unvisited', headCount: {'$gt': 0}})
                            .sort('-headCount')
                            .select('url')
                            .limit(limit)
                            .lean();
